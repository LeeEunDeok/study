...
public class MyUtility {
	public static String getCurrentTime() {
		// 현재 시각을 문자열로 만들어서 반환해줍니다.
		String pattern = "yyyy/DD/dd hh:mm:ss";
		return new SimpleDateFormat(pattern).format(new Date());
	}
	
	public static void deleteOldImageFile(String imageUploadWebPath, MultipartRequest mr) {
		// 웹서버 경로에 있는 과거 이미지를 삭제합니다.
		// 관련 파일 : prUpdateForm.jsp 파일에 name 속성이 "deleteImage숫자"인 항목들
		String[] deleteImages = {
	...
	public static MultipartRequest getMultipartRequest(HttpServletRequest request, String uploadPath) {
		 ...
		 * a.png 파일을 다시 업로드하면 a1.png 이름으로 업로드 됩니다. */
		
		try {
			mr = new MultipartRequest( // ※Shopping(이론) 233p
					request,
					uploadPath,
					maxPostSize,
					ENCODING,
